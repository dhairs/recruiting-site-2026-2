...
export default function InterviewScorecard({ 
  ...
  if (loading && !scorecardConfig) {
    return <div className="text-neutral-500">Loading interview scorecard...</div>;
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
        ...
        {isPrivilegedUser && allTeamSystems.length > 1 && (
            <div className="p-4 rounded-lg bg-neutral-900/50 border border-white/5">
                ...
                <p className="text-xs text-neutral-600 mt-2">
                    <span className="text-green-400">●</span> indicates systems with configured interview scorecards
                </p>
            </div>
        )}
        ...
        {aggregates && aggregates.totalSubmissions > 0 && (
            <div className="p-4 rounded-lg bg-gradient-to-br from-blue-500/10 to-indigo-500/5 border border-blue-500/20">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-bold">Interview Aggregate Scores</h3>
        ...
        {allSubmissions.length > 0 && (
            <div className="p-4 rounded-lg bg-neutral-900/50 border border-white/5">
                <h3 className="text-white font-bold mb-4">Interviewer Submissions</h3>
                <div className="space-y-3">
        ...
        <div className="border-t border-white/5 pt-6">
            <h3 className="text-white font-bold mb-4">Your Interview Scorecard</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
                {scorecardConfig.fields.map(field => (
                    <div key={field.id} className="p-4 rounded-lg bg-neutral-900 border border-white/5">
                                    ...
                                    <label key={val} className="flex flex-col items-center cursor-pointer group">
                                        ...
                                        <div className={clsx(
                                            ...
                                            scorecardData[field.id] === val 
                                              ...
                                              : "bg-neutral-800 text-neutral-400 border-white/10 group-hover:border-blue-500/50"
                                        )}>
                                            {val}
                                        </div>
                                    </label>
                        ...
                        {field.type === "text" && (
                            <input 
                              type="text" 
                              className="w-full bg-neutral-800 border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500"
                        ...
                        {field.type === "long_text" && (
                            <textarea 
                              className="w-full bg-neutral-800 border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-blue-500 h-24"
                              value={scorecardData[field.id] || ""}
                              onChange={(e) => setScorecardData(prev => ({ ...prev, [field.id]: e.target.value }))}
                            />
                        )}
                ...
                <div className="flex justify-end">
                    <button 
                      type="submit" 
                      disabled={saving}
                      className="px-6 py-2 bg-blue-600 text-white rounded font-medium hover:bg-blue-700 transition-colors disabled:opacity-50"
                    >
